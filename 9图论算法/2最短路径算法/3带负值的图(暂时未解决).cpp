//恩,程序在这里进行思考的是,当有路径为负值时的对应操作,书中提示的是把带权路径算法和无权算法结合起来能解决这个问题,应为我们要先把这个负值边取缔掉,也就是我们
//选取一个常数△然后将其值加入到每个边上从而来消除负边值,然后按照最短路径将其求出,在加入这个△即可.但是这会影响每个节点的最短路径值,应为在构建时当有一个节点
//W到另一个节点V上是有两条路径存在,一条是不经过节点,另一条是经过3个节点,但是三节点的最小,当我们加入△时,则会是最短路径出错.要不就在求出 
...
void Dijkstra1(Index S, Table T, Graph G)
{
	...
	{		
		...
		T[V].known = true;//标记此节点.应为这个节点上的dist值已经确定下来,并且是最小的. 
